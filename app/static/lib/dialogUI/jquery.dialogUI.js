...
//			bt_text: '<input type="button" value="' + g.blockUI_lang.button_sure + '" id="confirm_yes" width="50%" height="43" align="center" class="submit" />&nbsp;&nbsp;<input type="button" value="' + g.blockUI_lang.button_cancel + '" id="confirm_no" class="cancel" />'
	...
	function m(p) {
		...
		q, p || {});
		//\u6e29\u99a8\u63d0\u793a(温馨提示)
		var o = '<h3 class="title">\u63d0\u793a</h3><div id="block_content" class="content"><div class="detail">' + p.msg + '</div></div><div id="block_bottom" class="bottom">' + p.bt_text + "</div>";
		return o
	}
	g.blockUI.defaults = {
	...
	var d = !$.support.leadingWhitespace;
	function e(q, o) {
		...
		var D = o.baseZ;
		var y = !$.support.leadingWhitespace ? g('<iframe class="blockUI" id="JS_blockUI" style="z-index:' + (D++) + ';"></iframe>') : g('<div class="blockUI" id="JS_blockUI" style="z-index:' + (D++) + ';"></div>');
		...
		if (!$.support.leadingWhitespace || o.forceIframe) {
			y.css("opacity", 0)
		}
		var A = [y, v, r];
		var u = C ? g("body") : g(q);
		...
		if (!$.support.leadingWhitespace && o.showOverlay) {
			y.show()
		}
		if (o.fadeInTime) {
			if (o.showOverlay) {
